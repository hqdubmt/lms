...
import { SUBJECT_COLOR, SUBJECT_LABEL, LESSON_TYPE_COLOR, LESSON_TYPE_LABEL } from '@/constants/math';
...
export function TopicCard({ topic, busy, genBusy, quizBusy, varBusy, onDelete, onGenerateAll, onGenerateQuiz, onGenerateVariations }: Props) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center gap-4">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          ...
          {topic.textbook && (
            <span className="text-xs text-gray-500 bg-gray-50 border border-gray-200 px-1.5 py-0.5 rounded-md">{topic.textbook}</span>
          )}
      ...
      <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
        <button onClick={onGenerateAll} disabled={genBusy || (topic._count?.concepts ?? 0) < 2}
          title={(topic._count?.concepts ?? 0) < 2 ? 'Cần ít nhất 2 khái niệm' : 'AI tạo 4 loại bài tập'}
        ...
        {onGenerateVariations && (
          <button onClick={onGenerateVariations} disabled={varBusy || (topic._count?.concepts ?? 0) < 1}
