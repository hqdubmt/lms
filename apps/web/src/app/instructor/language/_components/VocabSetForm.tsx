...
export function VocabSetForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await api.post<{ id: string }>('/language/vocab-sets', {
        title, language: lang, level, description: desc,
      });
      router.push(`/instructor/language/vocab/${created.id}`);
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-3">
          <div className="sm:col-span-2">
            ...
            <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="VD: Từ vựng IELTS Band 6.0" required autoFocus />
          </div>
          <div>
          ...
          <div className="sm:col-span-2 flex gap-3">
            <Button type="submit" disabled={creating}>
              {creating && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Tạo và thêm từ
            </Button>
