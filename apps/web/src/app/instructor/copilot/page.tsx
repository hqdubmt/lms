...
export default function InstructorCopilotPage() {
  ...
  const convertFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch('/api/instructor/documents/preview', {
      ...
      while (true) {
        ...
        for (const line of chunk.split('\n')) {
          ...
          try {
            ...
            if (parsed.type === 'chunk' && parsed.text) setStreamText(prev => prev + parsed.text);
            if (parsed.type === 'done') {
          ...
          {state === 'converting' ? (
            ...
            <>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">PDF · DOCX · PPTX · XLSX · CSV · HTML · TXT</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <div className="flex items-center gap-2 min-w-0">
              ...
              {subject && (
                <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium shrink-0">{subject}</span>
              )}
              {quality != null && (
        ...
        <div className="space-y-4">
          ...
          {(state === 'generating' || state === 'done') && (
            <div className="space-y-3">
              ...
              {state === 'done' && <JsonViewer data={resultData} raw={streamText} />}
            </div>
          )}
        </div>
      )}
