...
export default function VietAnalyticsPage() {
  ...
  useEffect(() => {
    if (!ready) return;
    api.get<AnalyticsData>('/viet/analytics').then(setData).finally(() => setLoading(false));
  }, [ready]);
          ...
          <table className="w-full text-xs">
            <thead className="bg-gray-50 text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-2 text-center font-medium"><RefreshCw className="h-3 w-3 inline" /></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {data.logs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-2 text-center">{log.validLessons}/{log.totalLessons}</td>
                  <td className="px-4 py-2 text-center"><ScoreBadge score={log.avgQualityScore} /></td>
                  <td className="px-4 py-2 text-center"><span className={cn(log.hallucinationCount > 0 ? 'text-orange-600 font-medium' : 'text-gray-400')}>{log.hallucinationCount}</span></td>
                  <td className="px-4 py-2 text-center"><span className={cn(log.duplicateCount > 0 ? 'text-violet-600 font-medium' : 'text-gray-400')}>{log.duplicateCount}</span></td>
                  <td className="px-4 py-2 text-center"><span className={cn(log.repairCount > 0 ? 'text-blue-600 font-medium' : 'text-gray-400')}>{log.repairCount}</span></td>
                  <td className="px-4 py-2 text-center"><span className={cn(log.retryTotal > 0 ? 'text-yellow-600 font-medium' : 'text-gray-400')}>{log.retryTotal}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
