...
export default function InstructorConvertPage() {
  ...
  const convert = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch('/api/instructor/convert/file', {
        method: 'POST',
        headers,
        credentials: 'include',
        body: formData,
      ...
      if (!res.ok) { setError(json.error || `Lỗi ${res.status}`); return; }
      setResult(json);
    } catch (e: any) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">PDF · DOCX · XLSX · CSV · HTML · TXT · MD</p>
            </div>
          </>
        )}
      </div>
