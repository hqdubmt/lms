...
export default function FlashQuizPage() {
  ...
  const startGame = async () => {
    setLoading(true);
    try {
      const data = await api.get<{ questions: Question[] }>('/viet/game/flash-quiz?count=10');
  ...
  const submitGame = useCallback(async (finalAnswers: Answer[], finalStreak: number) => {
    ...
    setLoading(true);
    try {
      const res = await api.post<SubmitResult>('/viet/game/flash-quiz/submit', {
  ...
  if (state === 'idle') return (
    <div className="max-w-lg mx-auto px-4 py-10 space-y-6">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-yellow-400 to-orange-500 p-8 text-white text-center shadow-xl">
        ...
        <h1 className="text-3xl font-black">Trắc Nghiệm Chớp Nhoáng</h1>
        <p className="text-white/80 mt-1">Chọn đúng nghĩa của từ — càng nhanh càng tốt!</p>
      </div>
      <div className="rounded-xl border p-5 space-y-4 bg-white">
        <div className="text-sm text-gray-500 space-y-1.5">
          <p>⚡ Mỗi câu có <span className="font-semibold text-orange-600">{TIME_PER_Q} giây</span> — hết giờ tính sai</p>
          <p>🔥 Streak liên tiếp sẽ tăng XP thưởng</p>
          <p>✦ Mỗi câu đúng: <span className="font-semibold text-orange-600">+5 XP</span></p>
          <p>🏆 Hoàn hảo 100%: <span className="font-semibold text-orange-600">+30 XP thưởng</span></p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className={cn('rounded-2xl p-8 text-white text-center shadow-xl',
            ...
            : result.score >= 60 ? 'bg-gradient-to-br from-orange-400 to-amber-600'
            ...
            {maxStreak >= 3 && <p className="text-white/80 text-sm mt-1">🔥 Streak cao nhất: ×{maxStreak}</p>}
          </div>
          <div className="space-y-2">
            {answers.map((a, i) => {
              const q = questions[i];
              return (
                <div key={a.questionId} className={cn('rounded-xl border p-3 flex items-center gap-3',
                  a.correct ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200')}>
                  {a.correct
                    ? <CheckCircle2 className="w-4 h-4 text-green-500 shrink-0" />
                    : <XCircle className="w-4 h-4 text-red-500 shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">{q?.word}</p>
                    {!a.correct && (
                      <p className="text-xs text-gray-500 mt-0.5">
                        {a.answer ? <span>Bạn: <span className="text-red-600">{a.answer}</span> · </span> : <span className="text-red-500">Hết giờ · </span>}
                        Đúng: <span className="text-green-600">{q?.correctAnswer}</span>
                      </p>
                    )}
                  </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={startGame} className="flex-1 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold transition">Chơi lại!</button>
          </div>
        </>
      ) : null}
    </div>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
      ...
      <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
        <div className={cn('h-2 rounded-full transition-all duration-1000', timerPct > 50 ? 'bg-green-500' : timerPct > 25 ? 'bg-yellow-500' : 'bg-red-500')}
          style={{ width: `${timerPct}%` }} />
      </div>

      <div className="rounded-2xl border-2 border-gray-200 bg-white p-6 text-center">
        <p className="text-xs uppercase tracking-wider text-gray-400 mb-2">Từ / Thành ngữ</p>
        <p className="text-3xl font-black text-gray-900 mb-2">{q.word}</p>
        {q.example && <p className="text-sm text-gray-400 italic">"{q.example}"</p>}
      </div>
      ...
      <div className="grid grid-cols-1 gap-2.5">
        {q.options.map((opt) => {
          ...
          if (state === 'answered') {
            ...
            cls += ' border-gray-200 bg-white hover:border-orange-400 hover:bg-orange-50 text-gray-800 cursor-pointer active:scale-98';
          }
          return (
