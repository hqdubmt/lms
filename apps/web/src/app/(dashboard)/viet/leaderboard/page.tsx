...
export default function VietLeaderboardPage() {
  const router = useRouter();
  const [entries, setEntries] = useState<LeaderboardEntry[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get<LeaderboardEntry[]>('/viet/leaderboard')
      .then(setEntries)
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            <Trophy className="h-5 w-5 text-yellow-500" />Bảng xếp hạng Tiếng Việt
          </h1>
              ...
              <div key={entry.id}
                ...
                <div className="flex-shrink-0">
                  {entry.user.avatarUrl ? (
                    ...
                    <div className="h-10 w-10 rounded-full bg-red-100 flex items-center justify-center border-2 border-white shadow">
