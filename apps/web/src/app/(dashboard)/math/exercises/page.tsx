...
import { SUBJECT_LABEL, SUBJECT_COLOR, SUBJECT_OPTIONS, EXERCISE_TYPE_LABEL, EXERCISE_TYPE_OPTIONS, EXERCISE_ICONS } from '@/constants/math';
import { api } from '@/lib/api';
import { cn } from '@/lib/utils';

interface MathExercise {
  id: string; title: string; type: string; subject: string; grade: number; level: string;
  description?: string;
...
function ExercisesContent() {
  ...
  const setFilter = useCallback((key: string, value: string) => {
    ...
    router.push(`/math/exercises?${params.toString()}`);
  }, [router, searchParams]);
  ...
  useEffect(() => {
    ...
    api.get<MathExercise[]>(`/math/exercises?${params.toString()}`)
      .then(setExercises)
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">
            {type ? `Bài tập: ${EXERCISE_TYPE_LABEL[type] || type}` : 'Tất cả bài tập Toán'}
          </h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {EXERCISE_TYPE_OPTIONS.map(t => (
            <button key={t.value}
              ...
              className={cn('px-3 py-1.5 rounded-xl text-sm font-medium transition-colors',
                type === t.value ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              )}>{t.label}</button>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map(ex => {
            ...
            return (
              <button key={ex.id}
                onClick={() => router.push(`/math/exercise/${ex.id}`)}
                ...
                <div className="flex items-start justify-between mb-3">
                  <span className={cn('text-xs font-semibold px-2 py-1 rounded-lg flex items-center gap-1', SUBJECT_COLOR[ex.subject] || 'bg-gray-100 text-gray-600')}>
